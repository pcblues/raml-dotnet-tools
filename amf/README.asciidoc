...
The AMF Tools for .NET allows developers to easily integrate and consume APIs that expose a RAML definition, or create a new ASP.NET implementation using a contract-first approach from a previously created RAML definition. See http://raml.org for information on RAML (RESTful API Markup Language). 
...
* RAML 1.0/0.8 compatible endpoint
...
+
. Specify the URL of the RAML definition and click the *Go* button, use the *Upload* button to select the file from the local filesystem.
+
...
. Start Visual Studio 2017 and create a new ASP.NET Web Application.
...
+
. The dialog lets you create a RAML definition or import an existing one. If you import an existinadd-raml-contract-vs2015g one, click  the *Go* button to download the RAML definition from an URL, or browse to use a local copy from your file system. 
...
image::docimages/add-raml-contract-vs2015Screen.png[align="center", width=550]
